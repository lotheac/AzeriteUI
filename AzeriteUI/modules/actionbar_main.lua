...
Module.CreateBar = function(self, parent, width, height, padding, includeValue, includeBg, includeFg, includeGrid, includeBorder)
	...
	if includeBg then 
		...
		bg:SetAllPoints(bar)
		bg:SetTexture(getPath("statusbar-backdrop"))
	...
	if includeFg then 
		...
		fg:SetAllPoints(bar)
		fg:SetTexture(getPath("statusbar-normal-overlay")) 
		bar.Fg = fg
	end
	...
	if includeGrid then 
		...
		for i = 1,gridSize do 
			...
			shade:SetTexture(getPath("shade-square")) -- statusbar-normal-overlay -- 
			...
			shade:SetSize(gridWidth, height)
			shade:SetPoint("LEFT", gridOffset, 0)
			shade:SetTexture(getPath("statusbar-resource-overlay")) -- statusbar-normal-overlay -- shade-square
	...
	if includeBorder then 
		...
		border:SetBackdrop({
			edgeFile = getPath("border-tooltip"), -- 
			edgeSize = 32*sMod
		})
...
Module.SpawnBars = function(self)
	...
	xp:SetStatusBarTexture(getPath("statusbar-resource")) -- statusbar-normal
	...
	rested:SetStatusBarTexture(getPath("statusbar-normal"))
	...
	ap:SetStatusBarTexture(getPath("statusbar-resource")) -- statusbar-normal
